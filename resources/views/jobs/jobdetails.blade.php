                ...
                <div class="p-6 text-gray-900">
                    <div class="container mx-auto px-4 py-6">
                        ...
                        <div class="overflow-x-auto">
                            <table class="w-full bg-white border border-gray-200 rounded-md shadow-xs">
                                <thead>
                                    <tr class="bg-gray-100 border-b border-gray-300">
                                    ...
                                    @foreach ($job->items as $item)
                                        <tr class="border-b border-gray-200 hover:bg-gray-50">
                                            ...
                                            @if ($item->status_id == 1 || $item->status_id == 3)
                                                @if ($item->filteredResults != null)
                                                    <td class="py-2 px-4">
                                                        <textarea rows="5" cols="50" name="result{{ $item->id }}"
                                                            class="w-full border border-gray-300 rounded-sm px-3 py-2 focus:outline-hidden focus:ring-2 focus:ring-blue-500">{{ $item->resultsToString() }}</textarea>
                                                    </td>
                                                @else
                                            ...
                                            @if ($item->status_id == 1 && $item->filteredResults != null)
                                                <td class="py-2 px-4">
                                                    <form
                                                        action="{{ route('codeanalyzer.createissue', ['jobitems' => $item]) }}">
                                                        <button type="submit"
                                                            class="bg-blue-600 text-white px-4 py-2 rounded-sm hover:bg-blue-700 transition">Issue
                                                            aanmaken</button>
                                                    </form>
                                                </td>
                                            @else
